...
# Data1 : 사업년도를 Key로 하고 해당년도에 필요한 Data를 가지고 있는 3차원 데이터
...
# 사업년도별 수익률데이터 엑셀로 저장
with pd.ExcelWriter('C:/Users/S/Desktop/사업년도별수익률.xlsx') as writer:
...
# PBR 10분위 별 평균수익률 하나의 dataframe으로 정리 및 엑셀로 저장
my_dict = {}
for i in range(0,2) :
    my_columns = []
    df_stat_year = pd.DataFrame()
    for keys, values in dict_stat.items() :
        my_columns.append(keys)
        df_stat_year = pd.concat([df_stat_year, values.mean(axis=i)], axis=1, sort=False)
    df_stat_year.columns = my_columns
    my_dict["%s" %str(i)] = df_stat_year
with pd.ExcelWriter('C:/Users/S/Desktop/PBR_Size_포트폴리오평균수익률.xlsx') as writer:
    for keys, values in my_dict.items() :
        values.to_excel(writer, sheet_name= keys)


# 사업년도별 PBR,Size 기준 포트폴리오 수익률데이터 엑셀로 저장
with pd.ExcelWriter('C:/Users/S/Desktop/사업년도별_PBR_Size_포트폴리오수익률.xlsx') as writer:
    for keys, values in dict_stat.items() :
        values.columns = ["SIZE1", "SIZE2", "SIZE3", "SIZE4", "SIZE5", "SIZE6", "SIZE7", "SIZE8", "SIZE9", "SIZE10"]
        values.index = ["PBR1", "PBR2", "PBR3", "PBR4", "PBR5", "PBR6", "PBR7", "PBR8", "PBR9", "PBR10"]
        values.to_excel(writer, sheet_name= keys)
